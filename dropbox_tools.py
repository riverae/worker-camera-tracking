...
RUNPOD_POD_ID = None
LOCAL_FOLDER_PATH = '/workspace/VIDEOS'
LOCAL_OUTPUT_FOLDER_PATH = '/workspace/SCENES'
local_project_files = listdir(LOCAL_FOLDER_PATH)
DROPBOX_DOWNLOAD_FOLDER_PATH = '/RunPod_CameraTracking_Videos'
DROPBOX_UPLOAD_FOLDER_PATH = '/RunPod_CameraTracking_Output'
...
async def download_file_chunked(index, access_token):
    
    dbx, df = list_files(access_token)
...
async def upload_file_chunked(filename, access_token):
    """
    Uploads a large file to Dropbox in multiple chunks concurrently.
    """
    dbx = connect_to_dropbox(access_token)
    DROPBOX_ACCESS_TOKEN = dbx._oauth2_access_token    
    local_path = os.path.join(LOCAL_OUTPUT_FOLDER_PATH, filename)
...
async def main():
    ...
    parser.add_argument("--token", default=argparse.SUPPRESS, type=str)
    ...
    if "index" in args and "token" in args:
        await download_file_chunked(index=int(args.index), access_token=str(args.token))
    elif "filename" in args and "token" in args:
        await upload_file_chunked(args.filename, access_token=args.token)
    elif "token" in args:
        _, df = list_files(args.token)
        print(df)
